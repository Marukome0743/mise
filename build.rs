...
fn codegen_registry() {
    ...
    for (short, info) in &tools {
        ...
        let overrides = info
            ...
            .unwrap_or_default();
        let rt = format!(
            r#"RegistryTool{{short: "{short}", description: {description}, backends: &[{backends}], bins: &[{bins}], aliases: &[{aliases}], test: &{test}, os: &[{os}], idiomatic_files: &[{idiomatic_files}], detect: &[{detect}], depends: &[{depends}], env_paths: &[{env_paths}], overrides: &[{overrides}]}}"#,
