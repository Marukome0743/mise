...
impl<'a> TaskToolInstaller<'a> {
    ...
    pub async fn collect_tool_requests<'t>(
        ...
        for task in tasks {
            ...
            if let Some(task_cf) = task.cf(config) {
                requests.extend(
                    self.collect_tools_from_config_file(task_cf.clone(), &task.name)
                        .await?,
                );
            } else if let Some(config_root) = &task.config_root {
                requests.extend(self.collect_tools_from_dir(config_root, &task.name).await?);
            }
        }
