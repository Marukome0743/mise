...
impl Task {
    ...
    pub(crate) fn from_path_unrendered_with_cf(
        ...
        let info = parse_mise_header_toml(&body)?
            .into_iter()
    ...
    async fn resolve_generated_usage_spec(
        ...
        let env = match env {
            Some(env) => env.clone(),
            None => self.usage_command_env(config).await?,
        };
        let cwd = match cwd {
            Some(cwd) => cwd,
        ...
        let cache_key = crate::hash::hash_to_str(&(
            &self.config_source,
            &self.display_name,
            command,
            &cwd,
        ...
        let spec = cell
            .get_or_try_init(|| async {
                ...
                #[cfg(windows)]
                let program = crate::path::resolve_posix_shell_program_path(
                    Path::new(&program),
                    &filtered_env,
                )
    ...
    pub async fn parse_usage_spec_with_vars(
        ...
        let (mut spec, scripts) = if let Some(file) = self.file_path(config).await? {
            let spec = if self.usage_command.is_some() {
                self.resolve_generated_usage_spec(config, cwd.clone(), Some(&env))
                    .await?
            } else {
    ...
    pub async fn parse_usage_spec_for_display(&self, config: &Arc<Config>) -> Result<usage::Spec> {
        if self.usage_command.is_some() {
            return self.resolve_generated_usage_spec(config, None, None).await;
        }
