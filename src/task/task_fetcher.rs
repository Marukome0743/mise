    ...
    pub async fn fetch_tasks(&self, config: &Arc<Config>, tasks: &mut Vec<Task>) -> Result<()> {
        ...
        for t in tasks {
            if let Some(file) = &t.file {
                ...
                remote.args = original.args;
                remote.trailing_args = original.trailing_args;
                remote.show_args_in_prefix = original.show_args_in_prefix;
                remote.inherited_env = original.inherited_env;
                remote.overlay_env.extend(original.overlay_env);
                remote.overlay_vars.extend(original.overlay_vars);
