...
impl Toolset {
    ...
    fn compute_env_cache_key(&self, config: &Arc<Config>) -> Result<String> {
        ...
        for (_, tv) in &current_versions {
            ...
            {
                registry_env_context.insert(
                    format!("{}:{}", tool.short, entry.name),
                    (
                        entry.paths,
                        entry.os,
        ...
        let mut settings_hash = compute_settings_hash();
        if !registry_env_context.is_empty() {
            settings_hash.push_str("\nregistry-env-paths:");
            settings_hash.push_str(&format!("{registry_env_context:?}"));
        }
