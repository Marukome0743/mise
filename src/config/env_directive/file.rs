...
impl EnvResults {
    ...
    pub async fn file(
        ...
        for p in xx::file::glob(normalize_path(config_root, s.into())).unwrap_or_default() {
            ...
            // vars. dotenv is expanded inside `Self::dotenv` via dotenvy itself.
    ...
    async fn dotenv(p: &Path, acc: &TeraEnvMap, seed: bool) -> Result<EnvMap> {
        ...
        // Seed lines for cross-file vars. dotenvy checks the process env first and
        // then these same-file vars, so this only fills genuinely-missing refs.
