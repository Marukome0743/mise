...
fn has_managed_env_drift(diff: &EnvDiff, current: &EnvMap) -> bool {
    ...
    let current_counts = current
        ...
        .unwrap_or_default();
    let managed_counts = diff.path.iter().fold(
        std::collections::HashMap::<&PathBuf, usize>::new(),
        |mut counts, path| {
            *counts.entry(path).or_default() += 1;
            counts
    ...
    managed_counts.into_iter().any(|(path, expected_count)| {
        let current_count = current_counts.get(path).copied().unwrap_or_default();
...
mod tests {
    ...
    #[test]
    fn ignores_unmanaged_variables_and_path_order() {
        let diff = managed_diff();
    ...
    #[test]
    fn detects_missing_managed_path_occurrences() {
        let diff = managed_diff();
        let path = std::env::join_paths(["/managed/bin", "/user/bin"])
            .unwrap()
            .to_string_lossy()
            .into_owned();
