...
use reqwest::header::{
    ACCEPT_ENCODING, AUTHORIZATION, CONTENT_RANGE, CONTENT_TYPE, ETAG, HeaderMap, HeaderValue,
...
const PARTIAL_DOWNLOAD_STATE_VERSION: u8 = 1;
...
impl DownloadValidator {
    ...
    fn is_valid(&self) -> bool {
        match self {
            ...
            Self::LastModified(value) => !value.is_empty(),
        }
    }
}
...
pub(crate) fn cleanup_download_dir(path: &Path) -> Result<()> {
    ...
    for entry in entries {
        ...
        let paired_partial = name
            .strip_suffix(".mise-part.json")
            .is_some_and(|stem| path.join(format!("{stem}.mise-part")).is_file())
            || name
                .strip_suffix(".mise-part")
                .is_some_and(|_| path.join(format!("{name}.json")).is_file());
        if name.starts_with('.') && paired_partial {
            continue;
        }
        let path = entry.path();
    ...
    headers
        ...
        .or_else(|| {
            headers
                ...
                .map(|value| DownloadValidator::LastModified(value.to_string()))
        })
}
        ...
        loop {
                ...
                == StatusCode::PARTIAL_CONTENT
            {
                ...
                if start != offset
                    ...
                    || validator
                        .as_ref()
                        .is_some_and(|value| value != &state.validator)
                {
                    partial.clear()?;
                    if restarted_without_resume {
            ...
            if let Some(total_size) = total_size {
                ...
                if actual_size != total_size {
                    return Err(std::io::Error::new(
                        ...
                        format!(
                            "downloaded file size mismatch: expected {total_size} bytes, got {actual_size}"
                        ),
                    )
                    .into());
                }
