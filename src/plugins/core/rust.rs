...
impl Backend for RustPlugin {
    ...
    async fn is_install_satisfied(
        ...
        let runtime = RustRuntime::resolve(config).await?;
    ...
    async fn install_version_(&self, ctx: &InstallContext, tv: ToolVersion) -> Result<ToolVersion> {
        let runtime = RustRuntime::resolve(&ctx.config).await?;
    ...
    async fn list_bin_paths(
        ...
        Ok(vec![RustRuntime::resolve(config).await?.bin_dir])
    }
...
impl RustRuntime {
    async fn resolve(config: &Arc<Config>) -> Result<Self> {
        ...
        Ok(Self::from_paths(homes, &env::PATH))
    }

    fn from_paths(homes: RustHomes, paths: &[PathBuf]) -> Self {
        if !rustup_is_initialized(&homes)
            && !homes.explicit
            && let Some(bin_dir) = external_rustup_bin_dir(paths)
        {
            debug!(
...
fn external_rustup_bin_dir(paths: &[PathBuf]) -> Option<PathBuf> {
    paths
        .iter()
...
"#,
    ...
    fn rust_runtime_prefers_initialized_homes() {
        ...
        let runtime = RustRuntime::from_paths(homes.clone(), &[external]);
    ...
    fn rust_runtime_uses_complete_external_provider() {
        ...
        let runtime = RustRuntime::from_paths(homes, std::slice::from_ref(&external));
