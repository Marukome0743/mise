...
///
/// Only acts when the directory holds exactly one binary (an executable or a
/// platform-suffixed file) and cleaning actually changes its name, so
/// multi-binary archives and files without a platform suffix are left untouched.
/// See discussion #6532.
fn auto_clean_single_archive_binary(dir: &Path, tool_name: &str) -> eyre::Result<()> {
    // Count a file as a binary if it is executable or carries an OS/arch platform
    // suffix. This ignores metadata files shipped alongside the binary — e.g.
    // CHANGELOG or AUTHORS, which are neither and aren't covered by
    // should_skip_file — while still counting other real executables, so genuine
    // multi-binary archives are left untouched. See discussion #6532.
    let binaries = file::ls(dir)?
        .into_iter()
        .filter(|p| p.is_file())
        .filter(|p| {
            let name = p
                ...
                .to_string_lossy()
                .to_string();
            if should_skip_file(&name, true) {
                return false;
            }
            file::is_executable(p) || clean_binary_name(&name, Some(tool_name)) != name
        })
        .collect::<Vec<_>>();
    let [path] = binaries.as_slice() else {
        return Ok(());
    };
    let file_name = path.file_name().unwrap().to_string_lossy().to_string();
    let cleaned_name = clean_binary_name(&file_name, Some(tool_name));
    // The sole binary has no platform suffix; nothing to clean.
...
"#
    ...
    #[cfg(unix)]
    #[test]
    fn test_auto_clean_single_archive_binary_leaves_multi_executable_archive() {
        // One platform-suffixed executable plus one already-clean executable is a
        // genuine multi-binary archive and must be left untouched — the clean
        // executable still counts as a binary. See discussion #6532.
        let tmp = tempfile::tempdir().unwrap();
        let suffixed = tmp.path().join("tool-linux-x86_64");
        let clean = tmp.path().join("helper");
        std::fs::write(&suffixed, b"x").unwrap();
        std::fs::write(&clean, b"x").unwrap();
        file::make_executable(&suffixed).unwrap();
        file::make_executable(&clean).unwrap();
    ...
    #[test]
    fn test_auto_clean_single_archive_binary_ignores_metadata_sibling() {
        // A metadata file (e.g. CHANGELOG) shipped next to the single suffixed
        // binary must not suppress the rename: only suffix-bearing files count.
        // See discussion #6532.
        let tmp = tempfile::tempdir().unwrap();
        let extracted = tmp.path().join("tool-linux-x86_64");
        let changelog = tmp.path().join("CHANGELOG");
        std::fs::write(&extracted, b"x").unwrap();
        std::fs::write(&changelog, b"notes").unwrap();
        ...
        assert!(changelog.is_file(), "metadata should be left untouched");
    }
