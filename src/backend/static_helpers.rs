...
fn auto_clean_single_archive_binary(dir: &Path, tool_name: &str) -> eyre::Result<()> {
    // Only consider files that actually carry an OS/arch platform suffix (i.e.
    // whose cleaned name differs). This ignores metadata files shipped alongside
    // the binary — e.g. CHANGELOG or AUTHORS, which are not covered by
    // should_skip_file — that would otherwise make the archive look like it holds
    // more than one binary and suppress the rename. See discussion #6532.
    let candidates = file::ls(dir)?
        .into_iter()
        .filter(|p| p.is_file())
        .filter_map(|p| {
            let name = p
            ...
            let cleaned = clean_binary_name(&name, Some(tool_name));
            (cleaned != name).then_some((p, cleaned))
        })
        .collect::<Vec<_>>();
    let [(path, cleaned_name)] = candidates.as_slice() else {
        return Ok(());
    };
    ...
    file::make_executable(&dest)?;
    debug!(
        "Auto-cleaned archive binary {} -> {cleaned_name}",
        path.display()
    );
    Ok(())
}
...
"#
    ...
    fn test_auto_clean_single_archive_binary_ignores_metadata_sibling() {
        ...
        assert!(tmp.path().join("tool").is_file(), "binary should be cleaned");
