...
impl AsdfBackend {
    ...
    fn version_listing_cache_context(env_results: &EnvResults) -> Option<String> {
        if env_results.env.is_empty() && env_results.env_remove.is_empty() {
            return None;
        }
        let env = env_results
            .env
        ...
        Some(hash_to_str(&(env, &env_results.env_remove)))
    }

    fn script_man_for_version_listing(&self, env_results: &EnvResults) -> ScriptManager {
...
mod tests {
    ...
    fn env_results(entries: &[(&str, &str)], removals: &[&str]) -> EnvResults {
    ...
    fn version_listing_cache_context_is_stable_and_tracks_changes() {
        ...
        let changed = env_results(&[("TOKEN", "other"), ("CHANNEL", "stable")], &["REMOVE_ME"]);
        let changed_removal =
            env_results(&[("TOKEN", "secret"), ("CHANNEL", "stable")], &["OTHER"]);
    ...
    fn version_listing_script_manager_applies_config_env() {
        ...
        let results = env_results(&[("MISE_TEST_VERSION_CHANNEL", "private")], &["PATH"]);

        let sm = backend.script_man_for_version_listing(&results);

        assert_eq!(
            sm.env.get(&OsString::from("MISE_TEST_VERSION_CHANNEL")),
            Some(&OsString::from("private"))
        );
        assert!(!sm.env.contains_key(&OsString::from("PATH")));
    }
