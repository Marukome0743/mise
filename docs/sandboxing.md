...
**Limitation**: An allow-list entry has to exist when the sandbox is built. Landlock binds each rule to an open descriptor, so a path that has not been created yet cannot be named by one: mise warns that the rule was dropped, and the task is then denied access to that path. To let a task create something, allow the directory that will contain it.
