...
System and shared installations use the destination-local
`<INSTALLS>/<TOOL>/.http-tarballs/` cache described above. These caches are not
controlled by the installing user's `MISE_DATA_DIR` or `MISE_CACHE_DIR`.
